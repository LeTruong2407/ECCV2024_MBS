...
def make_model(opts, classes=None):
    if opts.backbone == "resnet50":
        ...
        ### Init model giống như thằng MBS
### init up head dùng để decoder - nó dùng base là vision transformer để làm trong bước decoder
### Còn bên paper MBS thì nó dùng thằng vision transformer + decoder là Mask Transformer để tạo ra các class
...
### Tận dụng được hết chỗ này (có thể dùng để sửa) 
...
### class này dùng để define từ model gốc nè
    def __init__(
        self,
        body,
        head,
...
## cái self.cls là một cái tensor có chiều sâu hay channels là c class. thì khi tính norm nó sẽ tính norm trên toàn bộ c_channels đó
## xong sau đó nó chia cho thằng khởi tạo cls mới để nó init trọng số cho nó dựa trên class cũ
    ...
    def _network(self, x, ret_intermediate=False, only_bg=False):
        ...
        if self.disable_background and self.in_eval:
            x_o[:, 0] = 0.
#### Cái này là code dùng cho SETR : Segmentatio Transformer
        #===xzy added for SETR
...
####### Hàm này đầu tiên nếu mô hình là transformer thì trả về 2 gía trị 
### nên ret có thể ra 2, còn nếu ko trans thì là 1
##### x_b đặc trưng được đưa qua backbone resnet hoặc swin-b thì sẽ được đưa vào Head 
### Head chính là deeplabv3 để lấy ra đặc trưng.
### tiếp theo ở vòng lặp, mod chính là từng class từ đầu đến cuối
### NẾu i = 0 tức là nó ở background và two_stage = false để mình apply pp bài báo.
### thì new_weigh_background là điều chỉnh trọng số nền mới bằng việc sử dụng trọng số nền cũ nhân với trọng số nền mới.
### Sau đó trọng số nền mới sẽ là nhân ma trận của embedding khởi tạo bg ở trên và new_weight_bg vừa tính
## thì task0_weight sẽ là nối nền mới vừa tạo được và những cái weight được khởi tạo
### sau đó out sẽ được sử dụng convolution với input = x_pl, weight là task0_weight, và bias là mod.bias
## Nếu i bắt đầu là class 1 (ko phải background nữa) thì chỉ cần mod(x_pl) tức là thực hiện phép tích chập với weight là class weight và bias là class bias
    ...
    def avg_weight_output(self, x):
        ...
        for i, mod in enumerate(self.cls[:-1]):
            if i == 0 and (not self.opts.two_stage):
                ...
                out.append(mod(x_pl))
### imprinting_w : nối trọng số của tất cả bộ phân loại cũ
### new_weight là class ở thời điểm mới nè
### thì nếu như two_stages = 0 (ko dùng pp cũ)
### Xét trên từng new class, đầu tiên nếu new_weight chưa được init nó sẽ bằng weight_new 
## Đầu tiên : weight new nó được khởi tạo bằng ma trận số 0 (có size là weigt_new hàng và class mới cột) nghĩa là bước đầu tiên trong transformation
### xong nó self.weight_new nó được init bằng kaiman. thì new_weight đầu tiên sẽ được tíbg bằng lấy thg feature self.weight_new * cho số class cũ (bước 1 )
## Tiếp theo nếu như nó là class thứ 2 trong tập class mới thì mình nối nó thôi, cũgn thực hiện y chang như vậy
### Sau đó thì cái out là đem thằng x_pl được trích xuất từ deep lab xong tích chập với new_weight với kernel_size =1
## Tức là ở đây mình có x_pl là  một cái feature map được trích xuất ra từ deeplab
### thực hiện convolution để nó có thông tin phân loại trên từng class vì mình có trọng số của các class. dòng 539
### Paper này nó dùng deeplab còn ở pp kia thì nó dùng thằng MaskTransformer để học ra các cls_token.
## x_o là nối tất cả đặc trưng phân loại lại, nó là vector logits. cho các lớp
        ...
        return x_o ## clear ở trên
## Cái này xzy là đang dùng cho model  transformer nè tức là nó dùng VIT được ở đây 
    # ==== xzy end ====

### Hàm này đầu tiên là nó lấy ra class mới nè là cls
### nếu như phương thức thông thường thì nó sẽ tạo ra cls mới bằng cách copu trọng số thông qua new_classifier_w vừa tạo ở trên. embedding vector khởi tạo randomly thôi.
### Thì chế độ của mình đang là not two_stages => nhảy xuống dứoi
###     Ở đây thì nó cũng tương tự tạo 1 cái imprinting_w (kĩ thuật dùng các weight cũ để set weight cho các lớp mới)
###     imprinting_w là nó tạo ra để ghép các trọng số của cls cũ vào với nhau
###     sau đó thì self.weight_old là trọng số cũ được tính toán ở trước sẽ được nhân với cái cls cũ khởi tạo bằng embedding. 
### Rồi nhân cho trọng số cls mới khởi tạo sẽ thu được new_weight.
##      sau khi đã có new_weight thì sẽ tiếp tục update nó vào trọng số của class mới cls. 
##          Tính new_weight_bg: Dùng trọng số nền (weight_old_bg) và imprinting_w[0] để điều chỉnh.
##      Chuẩn hóa bằng gamma_bg: Để đảm bảo chuẩn L2 của trọng số mới tương đương với trọng số cũ.
##      Gán lại cho lớp nền self.cls[0].
